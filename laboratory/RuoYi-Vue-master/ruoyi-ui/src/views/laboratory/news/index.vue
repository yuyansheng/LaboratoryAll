  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['laboratory:news:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['laboratory:news:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['laboratory:news:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['laboratory:news:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="newsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="删除" align="center" prop="isDelete">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_yes_no" :value="scope.row.isDelete"/>
        </template>
      </el-table-column>
...
<script>
import { listNews, getNews, delNews, addNews, updateNews } from "@/api/laboratory/news";
import member from "@/views/laboratory/member/index.vue";
...
export default {
  ...
  methods: {
    searchAuthors(query){
      listMember({pageNum:1,pageSize: 10,name:query}).then(response=>{
    ...
    getList() {
      ...
      listNews(this.queryParams).then(response => {
        ...
        this.newsList.forEach(news=>{
          getMember(news.authorId).then(response=>{
            console.log(typeof news.releaseTime)
            this.$set(news,'author',response.data.name);
          })
        })
      });
    },
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('laboratory/news/export', {
        ...this.queryParams
